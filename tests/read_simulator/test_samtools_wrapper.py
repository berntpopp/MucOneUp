...
class TestConvertBamToPairedFastq:
    """Test convert_bam_to_paired_fastq paired-end BAM to FASTQ conversion.

    Following Phase 2 testing principles:
    - Mock ONLY at system boundary (subprocess.run via run_command)
    - Test OUR code's logic (command construction, error handling, validation)
    - NOT testing samtools itself
    """

    # ========== HAPPY PATH TESTS ==========

    def test_constructs_correct_paired_fastq_command(self, mocker, tmp_path):
        """Test that samtools fastq command is constructed correctly for paired-end."""
        import gzip

        # Arrange
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "output_R1.fastq.gz"
        output_fq2 = tmp_path / "output_R2.fastq.gz"
        input_bam.write_bytes(b"FAKE_BAM_DATA")

        # Mock run_command to create outputs
        def mock_run_cmd(cmd, **kwargs):
            # Create valid outputs with same read count (gzipped because of .gz extension)
            fastq_content = "@read1\nACGT\n+\nIIII\n@read2\nTGCA\n+\nJJJJ\n"
            with gzip.open(output_fq1, "wt") as f:
                f.write(fastq_content)
            with gzip.open(output_fq2, "wt") as f:
                f.write(fastq_content)
        ...
        fq1_out, fq2_out = convert_bam_to_paired_fastq(
            samtools_cmd="samtools",
            input_bam=str(input_bam),
            output_fq1=str(output_fq1),
            output_fq2=str(output_fq2),
            options=FastqConversionOptions(threads=8, validate_pairs=True),
        )

        # Assert: Check command construction
        assert mock_run.call_count == 1
        ...
        assert "fastq" in cmd
        assert "-1" in cmd
        assert str(output_fq1) in cmd
        assert "-2" in cmd
        assert str(output_fq2) in cmd
        assert "-@" in cmd
        assert "8" in cmd
        assert "-n" in cmd  # preserve_read_names=True
        assert "-0" in cmd
        assert "/dev/null" in cmd
        assert str(input_bam) in cmd

        # Verify return values
        assert fq1_out == str(output_fq1)
        assert fq2_out == str(output_fq2)

    def test_uses_default_options_when_none_provided(self, mocker, tmp_path):
        """Test that default FastqConversionOptions are used when options=None."""
        import gzip

        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq.gz"
        output_fq2 = tmp_path / "R2.fq.gz"
        ...
        convert_bam_to_paired_fastq("samtools", str(input_bam), str(output_fq1), str(output_fq2))

        # Assert: Default options used
        cmd = mock_run.call_args[0][0]
        assert "-n" in cmd  # Default: preserve_read_names=True
        assert "-@" in cmd
        idx = cmd.index("-@")
        assert cmd[idx + 1] == "4"  # Default: threads=4

    def test_handles_singleton_output_file(self, mocker, tmp_path):
        """Test that singleton file is created when specified in options."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq.gz"
        output_fq2 = tmp_path / "R2.fq.gz"
        singleton_file = tmp_path / "singleton.fq.gz"
        input_bam.write_bytes(b"BAM")

        def mock_run_cmd(cmd, **kwargs):
            # Verify -s flag used (not /dev/null)
            assert "-s" in cmd
            assert str(singleton_file) in cmd
            assert "/dev/null" not in cmd

            # Create outputs
            fastq_content = "@r1\nA\n+\nI\n"
            output_fq1.write_text(fastq_content)
            output_fq2.write_text(fastq_content)
            singleton_file.write_text("@singleton\nACGT\n+\nIIII\n")
        ...
        # Act
        opts = FastqConversionOptions(output_singleton=singleton_file, validate_pairs=False)
        convert_bam_to_paired_fastq(
            "samtools", str(input_bam), str(output_fq1), str(output_fq2), opts
        )

        # Assert
        assert singleton_file.exists()

    def test_validates_read_pair_integrity_by_default(self, mocker, tmp_path):
        """Test that read pair validation is performed by default."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq"
        output_fq2 = tmp_path / "R2.fq"
        input_bam.write_bytes(b"BAM")

        def mock_run_cmd(cmd, **kwargs):
            # Create outputs with SAME read count (valid)
            output_fq1.write_text("@r1\nA\n+\nI\n@r2\nC\n+\nI\n")  # 2 reads
            output_fq2.write_text("@r1\nT\n+\nI\n@r2\nG\n+\nI\n")  # 2 reads
        ...
        # Act: Default options (validate_pairs=True)
        fq1, fq2 = convert_bam_to_paired_fastq(
            "samtools", str(input_bam), str(output_fq1), str(output_fq2)
        )

        # Assert: Should succeed (read counts match)
        assert Path(fq1).exists()
        assert Path(fq2).exists()

    def test_skips_validation_when_disabled(self, mocker, tmp_path):
        """Test that validation is skipped when validate_pairs=False."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq"
        output_fq2 = tmp_path / "R2.fq"
        input_bam.write_bytes(b"BAM")
        ...
        def mock_run_cmd(cmd, **kwargs):
            # Create outputs with DIFFERENT read counts (invalid but validation disabled)
            output_fq1.write_text("@r1\nA\n+\nI\n@r2\nC\n+\nI\n")  # 2 reads
            output_fq2.write_text("@r1\nT\n+\nI\n")  # 1 read (MISMATCH!)
        ...
        opts = FastqConversionOptions(validate_pairs=False)
        fq1, fq2 = convert_bam_to_paired_fastq(
            "samtools", str(input_bam), str(output_fq1), str(output_fq2), opts
        )

        # Assert: Should succeed (validation skipped, even though counts don't match)
        assert Path(fq1).exists()
        assert Path(fq2).exists()
    ...
    def test_validates_input_bam_exists(self, tmp_path):
        ...
        with pytest.raises(FileOperationError, match="Input BAM file not found"):
            convert_bam_to_paired_fastq(
                "samtools", str(missing_bam), str(output_fq1), str(output_fq2)
            )
    ...
    def test_validates_input_bam_not_empty(self, tmp_path):
        ...
        with pytest.raises(FileOperationError, match="Input BAM file is empty"):
            convert_bam_to_paired_fastq(
                "samtools", str(empty_bam), str(output_fq1), str(output_fq2)
            )

    def test_validates_r1_output_created(self, mocker, tmp_path):
        """Test that missing R1 output raises error."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq"
        output_fq2 = tmp_path / "R2.fq"
        input_bam.write_bytes(b"BAM")
        ...
        with pytest.raises(FileOperationError, match=r"R1 output.*not created"):
            convert_bam_to_paired_fastq(
                "samtools", str(input_bam), str(output_fq1), str(output_fq2)
            )

    def test_validates_r2_output_created(self, mocker, tmp_path):
        """Test that missing R2 output raises error."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq"
        output_fq2 = tmp_path / "R2.fq"
        input_bam.write_bytes(b"BAM")
        ...
        with pytest.raises(FileOperationError, match=r"R2 output.*not created"):
            convert_bam_to_paired_fastq(
                "samtools", str(input_bam), str(output_fq1), str(output_fq2)
            )

    def test_validates_r1_output_not_empty(self, mocker, tmp_path):
        """Test that empty R1 output is detected."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq"
        output_fq2 = tmp_path / "R2.fq"
        input_bam.write_bytes(b"BAM")
        ...
        with pytest.raises(FileOperationError, match=r"R1 output.*is empty"):
            convert_bam_to_paired_fastq(
                "samtools", str(input_bam), str(output_fq1), str(output_fq2)
            )

    def test_detects_read_pair_count_mismatch(self, mocker, tmp_path):
        """Test that read count mismatch is detected (critical for data integrity)."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "R1.fq"
        output_fq2 = tmp_path / "R2.fq"
        input_bam.write_bytes(b"BAM")
        ...
        def mock_run_cmd(cmd, **kwargs):
            # Create outputs with DIFFERENT read counts
            output_fq1.write_text("@r1\nA\n+\nI\n@r2\nC\n+\nI\n@r3\nG\n+\nI\n")  # 3 reads
            output_fq2.write_text("@r1\nT\n+\nI\n@r2\nG\n+\nI\n")  # 2 reads
        ...
        # Act & Assert
        with pytest.raises(FileOperationError, match=r"Read pair count mismatch.*3.*2"):
            convert_bam_to_paired_fastq(
                "samtools", str(input_bam), str(output_fq1), str(output_fq2)
            )

    def test_creates_parent_directories_if_missing(self, mocker, tmp_path):
        """Test that missing parent directories are auto-created."""
        input_bam = tmp_path / "input.bam"
        output_fq1 = tmp_path / "nested" / "dir1" / "R1.fq"
        output_fq2 = tmp_path / "nested" / "dir2" / "R2.fq"
        input_bam.write_bytes(b"BAM")
        ...
        # Assert parent dirs don't exist
        assert not output_fq1.parent.exists()
        assert not output_fq2.parent.exists()

        # Act
        convert_bam_to_paired_fastq("samtools", str(input_bam), str(output_fq1), str(output_fq2))

        # Assert: Directories created
        assert output_fq1.parent.exists()
        assert output_fq2.parent.exists()
        assert output_fq1.exists()
        assert output_fq2.exists()


class TestCountFastqReads:
    """Test _count_fastq_reads helper function."""

    def test_counts_reads_in_uncompressed_fastq(self, tmp_path):
        """Test counting reads in uncompressed FASTQ."""
        fastq = tmp_path / "test.fq"
        fastq.write_text("@read1\nACGT\n+\nIIII\n@read2\nTGCA\n+\nJJJJ\n@read3\nAAAA\n+\nKKKK\n")

        count = _count_fastq_reads(fastq)

        assert count == 3

    def test_counts_reads_in_gzipped_fastq(self, tmp_path):
        """Test counting reads in gzip-compressed FASTQ."""
        import gzip
        ...
        count = _count_fastq_reads(fastq_gz)

        assert count == 2

    def test_handles_empty_fastq(self, tmp_path):
        """Test that empty FASTQ returns 0 reads."""
    ...
    def test_raises_error_for_missing_file(self, tmp_path):
        ...
        missing_fastq = tmp_path / "nonexistent.fq"

        with pytest.raises(FileOperationError, match="Failed to count reads"):
            _count_fastq_reads(missing_fastq)
