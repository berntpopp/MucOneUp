...
class TestIntegrationWithRealData:
    ...
    def test_performance_benchmark(self, config, mutant_sample_path):
        ...
        # Should complete in reasonable time (lenient threshold for CI environments)
        # Typical runtime: < 1s locally, allow 30s for CI system load variability
        assert elapsed_time < 30.0, (
            f"Workflow took unexpectedly long: {elapsed_time:.3f}s (threshold: 30s)"
        )
